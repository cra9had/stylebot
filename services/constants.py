...
class Constants:
    PROMPT_TEMPLATES = {
        ENTER_PROMPT_TEMPLATE: """Ты бот-стилист. Запрос: "{user_prompt}". Если в запросе в кавычках есть задача подобрать образ и перечислена
одежда и стиль, выведи следующее: {{clothes: ["Одежда_1", "Одежда_2", ...], "style": стиль из запроса}}.
Если в запросе нет стиля, сформируй его сам. Если в запрос не указана одежда, выведи BadClothes.""",
...
случайные комбинации цвета и одежды, из тех, что указаны, и добавь в название одежды выбранный цвет. Верни 
